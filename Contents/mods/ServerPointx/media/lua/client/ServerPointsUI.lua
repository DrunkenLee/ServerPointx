...
function ServerPointsUI:setVisible(visible)
  ...
  if visible then
      ...
      sendClientCommand("ServerPoints", "get", { getPlayer():getUsername() })
  end
end
...
function ServerPointsUI.LoadType.REDEEMKILL(row, entry)
  ...
  row.texture = getTexture("media/ui/Moodle_Icon_CantSprint.png")
end

function ServerPointsUI.LoadType.BUFF(row, entry)
  row.quantity = entry.quantity or 1
  row.text = entry.target .. " -> BUFF"
  row.texture = getTexture("media/ui/Moodle_Icon_CantSprint.png")
end

function ServerPointsUI.LoadType.UTIL(row, entry)
  row.quantity = entry.quantity or 1
  if entry.target == "HEALTH" then
    row.text = "Heal Bitten and All Sickness"
    row.texture = getTexture("media/ui/iconInHotbar.png")
  elseif entry.target == "TRAITADD" then
    row.text = "Add Trait: " .. entry.trait
    row.trait = entry.trait
    row.texture = getTexture("media/ui/Moodle_internal_plus_green.png")
  elseif entry.target == "TRAITLOSE" then
    row.text = "Remove Trait: " .. entry.trait
    row.trait = entry.trait
    row.texture = getTexture("media/ui/Moodle_internal_minus_red.png")
  end
end
  ...
  if module == "ServerPoints" and command == "load" then
      ...
      for k, v in pairs(arguments) do
          ...
          for _, entry in ipairs(v) do
              ...
              row.trait = entry.trait -- Ensure the trait property is set
...
function validateTrait(player, trait)
  ...
  if conflict and player:HasTrait(conflict) then
      return false, "You need to remove the conflicting trait: " .. conflict .. " first."
  end
...
function ServerPointsUI.BuyType.UTIL(row)
  ...
  if row.target == "TRAITADD" then
      ...
      local isValid, message = validateTrait(player, trait)
      if not isValid then
          player:Say(message)
          return
      end
      ...
      if trait and not player:HasTrait(trait) then
          ...
          player:Say("You already have the trait: " .. trait)
      end
  end

  if row.target == "TRAITLOSE" then
      local trait = row.trait
      if trait and player:HasTrait(trait) then
          ...
          player:Say("You don't have the trait: " .. trait)
      end
  end
end
...
function ServerPointsUI.printPlayerTraits(player)
  ...
  for i = 0, traits:size() - 1 do
      table.insert(traitList, traits:get(i))
  end
  print("Player: " .. player:getUsername() .. " Traits: " .. table.concat(traitList, ", "))
end

function ServerPointsUI.BuyType.REDEEMKILL(row)
  local player = getPlayer()
  local killCount = player:getZombieKills() or 0
  local survivalTime = player:getHoursSurvived()
  local playerUsername = player:getUsername() or "admin"
...
function ServerPointsUI.BuyType.BUFF(row)
  ...
  if row.target == "RUN1" then
    SpeedFramework.SetPlayerSpeed(player, 1.5)
    player:Say("Sprint speed increased by 1.5%!")

    player:Say("Tier saya" .. playerTier)
  end

  if row.target == "RUN2" then
    SpeedFramework.SetPlayerSpeed(player, 2)
    player:Say("Sprint buffs added!")
  end

  if row.target == "ASPD1" then

    player:Say("Attack speed 50%")
  end

  if row.target == "ASPD2" then

    player:Say("Attack speed 100%!")
  end
end
